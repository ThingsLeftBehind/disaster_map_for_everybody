...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (!parsedDevice.success || !subscription) {
    return jsonError(res, 400, { ok: false, error: 'invalid_payload', errorCode: 'INVALID_PAYLOAD', message: '通知を有効にできませんでした。' });
  }
  ...
  try {
    ...
    console.warn('[push:subscribe] failed', { code, error: message });
    return jsonError(res, 500, { ok: false, error: 'subscribe_failed', errorCode: 'SUBSCRIBE_FAILED', message: '通知を有効にできませんでした。' });
  }
}
