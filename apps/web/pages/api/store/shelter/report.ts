...
import { reportComment } from 'lib/store/adapter';
import { ipHash } from 'lib/store/security';
import { ShelterReportBodySchema } from 'lib/store/types';
...
const REPORT_RATE_LIMIT = { keyPrefix: 'report:shelter_comment', limit: 10, windowMs: 10 * 60_000 };
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  try {
    ...
    const parsed = ShelterReportBodySchema.safeParse(req.body);
